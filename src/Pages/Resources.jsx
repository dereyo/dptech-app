...
const Resources = () => {
  ...
  // 💡 1. المصفوفة الاحتياطية (الفيك) القديمة بتاعتك زي ما هي بالظبط
  const fakeResources = [
    { id: 1, name: "Steel Beams", Icon: Package, CatIcon: Database, colorKey: "blue", cat: "Materials", qty: 120, max: 500, unit: "Tons", status: "Completed" },
    { id: 2, name: "Concrete Mix", Icon: Building2, CatIcon: Building2, colorKey: "teal", cat: "Construction", qty: 450, max: 500, unit: "m³", status: "In Progress" },
    { id: 3, name: "Solar Panels", Icon: Zap, CatIcon: Zap, colorKey: "amber", cat: "Energy", qty: 80, max: 200, unit: "Units", status: "Completed" },
    { id: 4, name: "Safety Helmets", Icon: Shield, CatIcon: Shield, colorKey: "coral", cat: "Equipment", qty: 200, max: 300, unit: "Pcs", status: "In Progress" },
    { id: 5, name: "Water Pipes", Icon: Droplet, CatIcon: Droplet, colorKey: "purple", cat: "Plumbing", qty: 300, max: 400, unit: "Meters", status: "Completed" },
  ];

  // 💡 2. محاولة جلب البيانات الحقيقية من السيرفر
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        ...
        // لو فيه بيانات حقيقية راجعة من الباك إند هندخلها مكان الفيك
        if (response.data && response.data.data && response.data.data.length > 0) {
          const mappedResources = response.data.data.map((proj, index) => {
            const colors = ['blue', 'teal', 'amber', 'coral', 'purple'];
            const icons = [Package, Building2, Zap, Shield, Droplet];
            ...
            return {
              ...
              colorKey: colorKey,
              cat: proj.location || "Construction", 
              qty: 100, 
              max: 100,
              unit: proj.projectCode || "Code", 
              status: "Completed", 
            };
          });

          setTableResources(mappedResources); // نضع الحقيقي
        } else {
          // لو السيرفر اشتغل بس لسه مفيش مشاريع، حط الفيك
          setTableResources(fakeResources);
        }
        setLoading(false);
      } catch (error) {
        console.error("خطأ أو السيرفر مش شغال - سيتم عرض البيانات الفيك:", error);
        // 💡 لو الباك إند مش شغال أو حصل أي خطأ، اعرض البيانات الفيك فوراً
        setTableResources(fakeResources);
        setLoading(false);
      }
    };
  ...
  // بقية البيانات الثابتة للأقسام التانية
  const stats = [
    ...
    { id: 4, title: "Total Resources", value: tableResources.length.toString(), subtext: "Across all stages", icon: "fa-boxes-stacked", gradient: "from-blue-50 to-blue-100", border: "border-blue-200", iconBg: "bg-blue-500", textMain: "text-blue-700", textDark: "text-blue-900" }
  ];
            ...
            <div key={cat.name} className={`bg-white border rounded-2xl p-5 ${border} ${cat.disabled ? "opacity-60" : ""}`}>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-3">
                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${cat.highlight ? "bg-orange-100 text-orange-600" : cat.disabled ? "bg-gray-100 text-gray-400" : "bg-green-100 text-green-600"}`}>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className={`text-xs font-semibold px-3 py-1 rounded-full ${cat.status === "Completed" ? "bg-green-100 text-green-700" : cat.status === "In Progress" ? "bg-orange-100 text-orange-700" : "bg-gray-100 text-gray-500"}`}>
                    {cat.status}
                  </span>
                </div>
              </div>
      ...
      <section className="mb-10 px-2 sm:px-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-5 animate-fade-up">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-gray-900">Projects & Resources</p>
          ...
          <div className="flex gap-2 justify-start sm:justify-end">
            <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium bg-green-50 text-green-700 border border-green-200">
              <CheckCircle className="w-3 h-3" /> {tableResources.length} Items
            </span>
          </div>
        </div>

        <div className="hidden md:grid grid-cols-[42px_2fr_1.4fr_1fr_1.3fr] px-4 py-3 mb-2">
          {["#", "Name / Project", "Category / Location", "Qty / Code", "Status"].map((h) => (
        ...
        <div className="flex flex-col gap-4">
          {tableResources.map((res, i) => {
            ...
            return (
              <div key={res.id} className="flex flex-col md:grid md:grid-cols-[42px_2fr_1.4fr_1fr_1.3fr] gap-3 md:gap-0 md:items-center p-5 md:px-4 md:py-4 bg-white border border-gray-100 rounded-2xl hover:bg-gray-50 hover:border-gray-200 transition-all shadow-sm relative">
                <span className="absolute top-4 right-4 md:static w-7 h-7 rounded-lg bg-gray-100 flex items-center justify-center text-[11px] text-gray-400 font-medium">
                  {i + 1}
                </span>
                ...
                <div className="flex items-center gap-2.5 pr-8 md:pr-0">
                  ...
                  <span className="text-[14px] md:text-[13px] font-bold md:font-medium text-gray-900">{res.name}</span>
                </div>
                ...
                <div className="w-full md:w-auto">
                  <span className="text-sm font-medium text-gray-800">
                    {typeof res.qty === 'number' && res.unit !== 'Code' ? (
                      <>
                        {res.qty} <span className="text-[11px] text-gray-400 font-normal">{res.unit}</span>
                      </>
                    ) : (
                      <>Code: <span className="text-[12px] text-gray-500 font-bold">{res.unit}</span></>
                    )}
                  </span>
                  <div className="mt-1.5 h-1.5 w-full md:w-24 rounded-full bg-gray-100 overflow-hidden">
                    <div className={`h-1.5 rounded-full transition-all duration-1000 ${c.bar}`} style={{ width: `${Math.round(res.qty / res.max * 100)}%` }} />
                  </div>
                </div>

                <div className={`flex items-center gap-2 pt-2 md:pt-0 border-t md:border-t-0 border-gray-50 w-full md:w-auto ${done ? "text-green-700" : "text-amber-700"}`}>
                  ...
                  <span className="text-xs font-bold md:font-medium">{res.status}</span>
                </div>
              </div>
            );
          })}
      ...
      <section className="mb-10 text-left">
        ...
        {['All', 'Completed', 'In Progress', 'Pending'].map((f) => (
          <button
            ...
            className={`mr-2 mb-4 px-4 py-1.5 rounded-full text-xs font-semibold border transition-all ${activeTab === f.toLowerCase().replace(' ', '-') || (f === 'All' && activeTab === 'all') ? 'bg-blue-900 text-white border-blue-900' : 'bg-white text-gray-500 border-gray-200 hover:border-gray-400'}`}
          >
            {f}
          </button>
        ))}
          ...
          {timelineEvents
            ...
            .map((event, i, arr) => (
              <div key={event.id} className="flex gap-4 relative">
                {i < arr.length - 1 && <div className="absolute left-[14px] top-8 bottom-0 w-px bg-gray-200 z-0" />}
                <div className={`z-10 mt-1 w-7 h-7 rounded-full flex items-center justify-center shrink-0 ${event.status === 'completed' ? 'bg-green-100 text-green-600' : event.status === 'in-progress' ? 'bg-orange-100 text-orange-500' : 'bg-gray-100 text-gray-400'}`}>
                  <i className={`fa-solid ${event.icon} text-xs`}></i>
                </div>
                <div className={`flex-1 mb-5 bg-white border rounded-2xl p-4 ${event.status === 'in-progress' ? 'border-orange-300 ring-1 ring-orange-200' : event.status === 'pending' ? 'border-gray-200 opacity-60' : 'border-gray-200'}`}>
                  <div className="flex items-start justify-between gap-3 mb-2">
                    <p className="font-semibold text-blue-900 text-sm">{event.title}</p>
                    <span className={`text-xs font-bold px-3 py-1 rounded-full shrink-0 ${event.status === 'completed' ? 'bg-green-100 text-green-700' : event.status === 'in-progress' ? 'bg-orange-100 text-orange-700' : 'bg-gray-100 text-gray-500'}`}>
